...
def zero_filled_pt190(x, mask, mod=False, norm=False):
    '''
    :param x: tensor of size (bs, C, H, W), C=1 for real image, C=2 for complex image
    :param mask: 1,H,W,1
    '''
    ...
    x_real = x
    x_imag = torch.zeros_like(x_real)
    x_cpx = torch.complex(x_real, x_imag)
    x_kspace = torch.fft.fft2(x_cpx, dim=(1, 2)) # torch.Size([1, 256, 256, 1])
    y_kspace = x_kspace * mask
    xu = torch.fft.ifft2(y_kspace, dim=(1, 2))

    if not mod:
        xu_ret = xu.real
    else:
        xu_ret = torch.sqrt(xu.real ** 2 + xu.imag ** 2)
    xu_ret = xu_ret.view(x_dim_0, x_dim_1, x_dim_2, x_dim_3)
